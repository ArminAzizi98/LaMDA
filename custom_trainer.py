...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                self.gradual_freeze_B(model, total_iter, max_steps, args.gradient_accumulation_steps, eta=0.3)
    ...
    def gradual_freeze_B(self, model, total_iters, max_steps,gradient_accumulation_steps, eta = 0.3):

        for pname, p in model.named_parameters():
            if "lora_B" in pname and ('weight' in pname):
                 rank = p.shape[1]
                 t = int(total_iters/(eta * max_steps * gradient_accumulation_steps))
                 if t>= rank:
                    p.requires_grad = False
                 else:
                    p.grad[:,:t] = p.grad[:,:t] * 0.0
